...
class CajaFactory(type):
    """CajaFactory creates specific types of Cajas depending on wrapped collection type."""
    ...
    def trait_filters(klass) -> frozenset:
        ...
        if klass not in k.traits_cache.keys():
            ...
            k.traits_cache[klass] = \
                frozenset(map(itemgetter(1), inspect.getmembers(collections.abc, tfilter)))
    ...
    def wrapper_type(content):
        """Return the Caja type that implements the same ifaces as the wrapped collection object"""
        k = CajaFactory
